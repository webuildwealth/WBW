 ...
 *    HUBSPOT_OWNER_ID       optional, zustaendiger Mitarbeiter, z. B. "82906351"
 *    HUBSPOT_FUNNEL_FELDER  optional, JSON-Mapping Funnel-Feld → HubSpot-Property
 * ========================================================================== */

'use strict';

const { kurz, legeLeadAn, baueLead } = require('./hubspot.js');
...
async function verarbeiteLead(body, env) {
  env = env || {};
  const key = env.HUBSPOT_TOKEN;
  if (!key) {
    console.error('lead: HUBSPOT_TOKEN ist nicht gesetzt');
  ...
  // Optionales Mapping einzelner Funnel-Antworten auf echte HubSpot-Properties.
  // Ohne Mapping stehen sie weiterhin vollstaendig in der Notiz — das Mapping
  // hebt nur die heraus, nach denen du spaeter filtern oder auswerten willst.
  if (env.HUBSPOT_FUNNEL_FELDER) {
    try {
      const mapping = JSON.parse(env.HUBSPOT_FUNNEL_FELDER);
      const quelle = {};
      ...
      Object.keys(mapping).forEach(function (feld) {
        const property = mapping[feld];
        // Nur schreiben, was wir kennen, und nur in gueltige Property-Namen.
        // Der Wildwuchs eines falsch gepflegten Mappings darf keinen Lead kippen.
        if (quelle[feld] && /^[a-z][a-z0-9_]{0,99}$/.test(property)) {
          lead.kontakt[property] = quelle[feld];
        }
      });
    } catch (e) {
      console.error('lead: HUBSPOT_FUNNEL_FELDER ist kein gueltiges JSON');
    }
  }

  /* --------------------------------------------------------------- Senden */
  try {
    const kontaktId = await legeLeadAn(key, lead, baueNotiz(d, antworten));
    return antwort(200, { ok: true, lead_id: kontaktId });

  } catch (e) {
    // Bewusst ohne personenbezogene Daten, damit die Funktionslogs sauber bleiben.
    console.error('lead: Anlage in HubSpot fehlgeschlagen —', e.message, '|', e.detail || '');
