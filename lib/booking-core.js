...
/* Antworten aus einem vorgeschalteten Fragebogen (beratung.html schickt sie
   mit). Erwartet wird eine Liste aus { feld, wert }; alles andere wird
   verworfen. Dadurch kann der Aufrufer weder die Struktur der Notiz noch die
   Länge des Kalendereintrags bestimmen — die Grenzen stehen hier. */
function leseAntworten(rohe) {
  if (!Array.isArray(rohe)) return [];
  return rohe.slice(0, 20)
    .map(function (a) {
      return { feld: kurz(a && a.feld, 80), wert: kurz(a && a.wert, 300) };
    })
    .filter(function (a) { return a.feld && a.wert; });
}
...
async function bucheTermin(body, env) {
  ...
  const antworten = leseAntworten(d.antworten);
  const zeilen = [
  ...
  if (antworten.length) {
    zeilen.push('', 'Angaben aus dem Kurzcheck:');
    antworten.forEach(function (a) { zeilen.push('  • ' + a.feld + ': ' + a.wert); });
  }
  ...
  if (env && env.CLOSE_API_KEY) {
    try {
      ...
      leadId = await legeLeadAn(env.CLOSE_API_KEY, lead, [
        ...
        antworten.length
          ? ['ANGABEN AUS DEM KURZCHECK'].concat(antworten.map(function (a) {
              return '  • ' + a.feld + ': ' + a.wert;
            })).concat([''])
          : []
      ).concat(
