...
const { legeLeadAn, baueLead } = require('./hubspot.js');
const { versende } = require('./mail.js');
const { bestaetigung, baueIcs } = require('./termin-mails.js');
...
async function bucheTermin(body, env) {
  ...
  const termin = await calApi(
    '/calendars/' + encodeURIComponent(k.kalender) + '/events',
    token,
    {
      method: 'POST',
      body: JSON.stringify({
        summary: 'Erstgespräch · ' + person,
        ...
        /* Die Erinnerungen lesen von hier. Die Beschreibung zu zerpflücken
           würde auch gehen, aber ein Freitext, auf den Code angewiesen ist,
           zerbricht beim ersten Mal, wenn jemand ihn umformuliert. */
        extendedProperties: {
          private: {
            lead_email: email,
            lead_vorname: vorname,
            lead_nachname: nachname,
            lead_name: person,
            lead_telefon: telefon || ''
          }
        }
      })
    }
  ...
  /* ---------------------------------------------------- Bestätigungsmail
     Bis hierher bekam der Interessent nur die Danke-Seite zu sehen. Wer den
     Tab schließt, hat damit nichts mehr in der Hand — kein Datum, keine
     Uhrzeit. Das Dienstkonto darf ohne Domain-Wide Delegation keine
     Kalendereinladung verschicken, deshalb geht die Bestätigung über den
     Mailversand hinaus, mit der Termindatei im Anhang.

     Wie beim CRM gilt: protokollieren, nicht scheitern. Der Termin steht
     bereits im Kalender; ein Ausfall des Mailanbieters ist kein Grund, dem
     Interessenten die zugesagte Uhrzeit wieder wegzunehmen. */
  let bestaetigungRaus = false;
  try {
    const mail = bestaetigung({
      vorname: vorname, nachname: nachname, start: start,
      zone: k.zone, dauer: k.dauer, telefon: telefon
    }, env);
    ...
    await versende(env, {
      ...
      anhang: {
        ...
        inhalt: baueIcs({
          start: start, ende: ende, terminId: termin.id,
          titel: 'Erstgespräch Finanzen & Medizin',
          beschreibung: 'Telefonisches Erstgespräch mit Benedict Hintz.'
        })
      }
    });
    bestaetigungRaus = true;
  } catch (e) {
    // Bewusst ohne personenbezogene Daten.
    console.error('booking: Bestätigungsmail fehlgeschlagen —', e.message, '|', e.detail || '');
  }
  ...
  let leadId = null;
  if (env && env.HUBSPOT_TOKEN) {
    try {
      const lead = baueLead(d, 'Terminbuchung über finanz-medizin.com', env);
      leadId = await legeLeadAn(env.HUBSPOT_TOKEN, lead, [
      ...
      // Bewusst ohne personenbezogene Daten.
      console.error('booking: Lead in HubSpot fehlgeschlagen —', e.message, '|', e.detail || '');
    }
  }

  return antwort(200, {
    ok: true, termin_id: termin.id, beginn: start.toISOString(), lead_id: leadId,
    /* Die Danke-Seite kann damit entscheiden, ob sie "Bestätigung ist
       unterwegs" schreibt oder den Termin selbst noch einmal groß anzeigt. */
    bestaetigung_versendet: bestaetigungRaus
  });
}

/* zugriffstoken, calApi und inZone werden von erinnerungen-core.js
   mitbenutzt. Eine zweite Google-Anmeldung waere eine zweite Stelle,
   an der ein abgelaufener Schluessel repariert werden muss. */
module.exports = { freieSlots, bucheTermin, konfig, zugriffstoken, calApi, inZone };
