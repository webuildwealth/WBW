...
/* ------------------------------------------------------- Termin auslesen */
/* Drei Wege fuehren zu einem Termin, und jeder legt die Kontaktdaten woanders ab:
 *
 *   1. Zusatzfelder  — das Buchungsformular auf der Website schreibt sie dorthin
 *   2. Teilnehmer    — wenn Benedict von Hand ueber Google Kalender einlaedt
 *   3. Beschreibung  — aeltere Buchungen, vor Einfuehrung der Zusatzfelder
 *
 * Ohne Weg 2 bekaeme jeder von Hand eingeladene Termin keine Erinnerung, weil
 * dort weder Zusatzfelder noch ein Beschreibungsblock existieren. */
function ausBeschreibung(text, feld) {
  const treffer = new RegExp('^' + feld + ':\\s*(.+)$', 'im').exec(String(text || ''));
  return treffer ? treffer[1].trim() : '';
}

/* Der eingeladene Gast — nicht der Kalenderbesitzer und kein Besprechungsraum.
   `self` markiert das eigene Konto, `organizer` den Einladenden; beide koennen
   je nach Kalender gesetzt sein oder fehlen, deshalb wird auf beides geprueft. */
function gast(eintrag) {
  const liste = eintrag.attendees || [];
  let abgesagt = false;
  for (const a of liste) {
    if (!a || !a.email) continue;
    if (a.self || a.organizer || a.resource) continue;
    if (a.responseStatus === 'declined') { abgesagt = true; continue; }
    return { gast: a, abgesagt: false };
  }
  return { gast: null, abgesagt: abgesagt };
}
...
function lieseTermin(eintrag) {
  ...
  const gefunden = gast(eintrag);
  const eingeladen = gefunden.gast || {};

  const email = (privat.lead_email || eingeladen.email ||
                 ausBeschreibung(beschreibung, 'E-Mail')).toLowerCase();
  const name = privat.lead_name || eingeladen.displayName ||
               ausBeschreibung(beschreibung, 'Name');
  ...
  return {
    ...
    /* Wer abgesagt hat, braucht keine Erinnerung — und darf auch nicht alle
       15 Minuten als Fehler im Protokoll auftauchen. */
    abgesagt: gefunden.abgesagt,
...
async function sendeFaellige(env, jetzt) {
  ...
  for (const eintrag of (liste.items || [])) {
    ...
    if (termin.abgesagt) { ergebnis.uebersprungen++; continue; }
